...
def test_interval_hour(time_):
    """
    Tests the time difference between the time last copy was made
...
def test_create_time_config(time_):
    ...
    assert list(params.keys()) == [
        'date', 'time_in', 'last_checked', 'content_size_1hr',
        'content_size_24hr'
    ]
...
def test_load_policy(policy):
    """Test creation and loading of the copy policy."""
...
def test_keyboard_activity(keyboard):
    ...
    pyautogui.press('b')
    pyautogui.press('c')
    keyboard.stop(timeout=1)
    assert keyboard._key_stroke_count == 3
